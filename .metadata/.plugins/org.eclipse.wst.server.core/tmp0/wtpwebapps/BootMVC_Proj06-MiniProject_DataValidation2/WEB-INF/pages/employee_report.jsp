...
<body>
	...
	<c:choose>
		<c:when test="${!empty emp.getContent()}">
			<table border="1" align="center" bgcolor="green">
				...
				<c:forEach  var="e" items="${emp.getContent()}">
					<tr>
					<td> ${e.empId}</td>
					<td> ${e.empName}</td>
					<td> ${e.designation}</td>
				...
				<tr>
					<td align="center" colspan="7"><a href="delete-all">Delete All</a></td>
				</tr>
			</table>
			<p style="text-align: center;">
				<c:if test="${!emp.isFirst()}">
					<a href="report?page=0&size=${emp.size}">First</a> &nbsp; &nbsp;
				</c:if>
				<c:if test="${emp.hasPrevious()}">
					<a href="report?page=${emp.getPageable().getPageNumber()-1}&size=${emp.size}">Previous</a> &nbsp; &nbsp;
				</c:if>
				<c:forEach var="i" begin="1" end="${emp.getTotalPages()}" step="1">
					<a href="report?page=${i-1}&size=${emp.size}">${i}</a> &nbsp; &nbsp;
				</c:forEach>
				<c:if test="${!emp.isLast()}">
					<a href="report?page=${emp.getTotalPages()-1}&size=${emp.size}">Last</a> &nbsp; &nbsp;
				</c:if>
			</p>
		</c:when>
		<c:otherwise>
